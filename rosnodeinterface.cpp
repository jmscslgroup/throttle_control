...
// Generated 25-May-2021 22:41:56
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"throttle_control\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(throttle_control_M, (NULL));
        throttle_control_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(throttle_control_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(throttle_control_M)
                == (NULL)) || rtmGetStopRequested(throttle_control_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(throttle_control_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
